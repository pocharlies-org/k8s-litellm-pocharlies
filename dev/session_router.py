...
async def _apply(data, requested_model, tracker, resident_ready, info):
    ...
    # Desborde de la compañía (23-09-2026, deroga el «la compañía nunca salta» del 21-09):
    # con su interruptor «Fallback Alibaba» encendido (company.alibaba, el que ya existe),
    # una sesión company que encuentra el residente lleno (local_slots en vuelo) se reescribe
    # a Alibaba en vez de encolar. Va por su interruptor, NO por el instant_reject global:
    # el global sigue gobernando al resto de clientes. Medido la noche del 22→23-09: la
    # compañía encolada contra un vLLM a 16 en curso tuvo el freno de saturación puesto el
    # 68 % del tiempo y 9 CTO parados; el fallback no entraba porque solo cubría errores.
    company_overflow = company and (config.get("company") or {}).get("alibaba") is True
    info["company_overflow"] = company_overflow
    ...
    # Las sesiones de la compañía (x-claude-class: company) usan SU válvula:
    # la gobierna company.alibaba (desbordan) y no el instant_reject global. Con
    # company.alibaba=false llegan selladas y no pasan de «sellada». Sellado,
    # uncensored, plan explícito, sticky ya ligado y re-bind por residente
    # no-ready aplican igual — la cabecera no esquiva precedencia.
    valvula, motivo = (company_overflow, "company_overflow") if company else (
        config["instant_reject"], "instant_reject")
    if valvula and model == RESIDENT_MODEL:
        ...
        if inflight is not None and inflight >= config["local_slots"] and _overflow_ok_info(info):
            if config["sticky"] and sid:
                await _sticky_set(sid, "alibaba")
            info["decision"] = motivo
            _rewrite(data, sid, motivo)
            log.warning(
                "session_router: %s sid=%s en_vuelo=%s slots=%s -> %s",
                info["decision"], sid or "-", inflight, config["local_slots"], OVERFLOW_MODEL,
            )
            return True
